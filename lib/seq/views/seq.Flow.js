 ...
 * 
 * @method
 * @param {Integer} start Index where insertion began
 * @param {seq.Block[]} items Blocks that were inserted
 */
seq.Flow.prototype.onInsertItems = function( start, items ) {
	...
	if ( $children.length ) {
		if ( start >= $children.length ) {
			...
			this.$.children().eq( start ).before( $items );
		} else {
			this.$.prepend( $items );
		}
	} else {
 ...
 * 
 * @method
 * @param {Integer} start Index where removal began
 * @param {seq.Block[]} items Blocks that were removed
 */
seq.Flow.prototype.onRemoveItems = function( start, items ) {
	_.each( items, function( item ) {
