...
seq.Transaction.transform = function( a, b ) {
	...
	for ( var i = 0, length = Math.max( aOps.length, bOps.length ); i < length; i++ ) {
		...
		if (
			// Same type
			aOp.constructor === bOp.constructor &&
			// Same index
			aOp.index === bOp.index &&
			// Same data length
		...
		if ( aOp instanceof seq.InsertOperation ) {
			if ( bOp instanceof seq.InsertOperation ) {
				// insert|insert
				if ( aOpL <= bOpL ) {
			...
			} else if ( bOp instanceof seq.RemoveOperation ) {
				// insert|remove
			}
		} else if ( aOp instanceof seq.RemoveOperation ) {
			if ( bOp instanceof seq.InsertOperation ) {
				// remove|insert
			} else if ( bOp instanceof seq.RemoveOperation ) {
				// remove|remove
