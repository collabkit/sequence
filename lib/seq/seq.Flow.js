...
seq.Flow.prototype.splice = function( index, howMany ) {
	...
	// Remove/insert
	var removed = this.blocks.splice.apply( this.blocks, [index, howMany].concat( insert ) );
...
seq.Flow.prototype.sort = function( compareFunction ) {
	// Reverse
	this.blocks.sort( compareFunction );
	// Synchronize
	this.disableAnimation();
	this.$.append.apply( this.$, _.pluck( this.blocks, '$' ) );
	this.enableAnimation();
